...
from .task_graph import get_graph
...
class Scheduler(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().debug("SCHEDULER INIT")
    ...
    def get_bandwidth(self, n1: str, n2: str) -> float:
        ...
        if ptime_1 == 0 and ptime_2 == 0:
            self.get_logger().debug("No bandwidth data available....")
        if ptime_1 > ptime_2:
    ...
    def output_callback(self, msg: String) -> None:
        try:
            ...
            self.get_logger().debug(f"COMPUTED MAKESPAN: {makespan.data}")
    ...
    def get_schedule(self) -> Dict[str, str]:
        try:
            ...
            self.get_logger().debug(f"{num_machines} machines")
            self.get_logger().debug(f"{num_tasks} tasks")
            task_graph_ids = {
                task: i
            ...
            self.get_logger().debug("COMM:")
            self.get_logger().debug(str(comm))
            comp = torch.Tensor([
                [
            ...
            self.get_logger().debug("COMP:")
            self.get_logger().debug(str(comp))
    ...
    def execute(self) -> None:
        ...
        self.get_logger().debug(f"SCHEDULE: {pformat(schedule)}")
        ...
        for task in self.graph.start_tasks():
            ...
            msg = String()
            msg.data = message
            self.get_logger().debug(f"SENDING INITIAL TASK {task} TO {node}")
            self.executor_topics[node].publish(msg)

    async def draw_task_graph(self) -> None:
        try:
            start = time.time()
            self.get_logger().debug("Drawing taskgraph")
            ...
            self.get_logger().debug(str(list(graph.nodes)))
            types = {
                t: i for i, t in enumerate(self.all_nodes)
            }
            self.get_logger().debug("types "+str(types))
            self.get_logger().debug("nodes "+str(self.all_nodes))
            ...
            try:
                ...
                return
            self.get_logger().debug("color "+str(node_color))
            ...
            self.get_logger().debug(f"publishing task graph image for visualization {time.time()-start} seconds")
            plt.close(fig)
        except:
            self.get_logger().error(traceback.format_exc())
        finally:
            self.get_logger().debug(f"leaving task graph drawer")
